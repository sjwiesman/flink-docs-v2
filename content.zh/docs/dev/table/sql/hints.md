...
<b>注意：</b>动态表选项默认值禁止使用，因为它可能会更改查询的语义。你需要将配置项 `table.dynamic-table-options.enabled` 显式设置为 `true`（默认值为 false），请参阅 <a href="{{< ref "/dev/table/config.zh" >}}">Configuration</a> 了解有关如何设置配置选项的详细信息。
