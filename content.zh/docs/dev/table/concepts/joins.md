...
Join 在批数据处理中是比较常见且广为人知的运算，一般用于连接两张关系表。然而在[动态表]({{< ref "docs/dev/table/streaming/dynamic_tables" >}})中 Join 的语义会难以理解甚至让人困惑。
...
如果一个 Join 限定输入[时间属性]({{< ref "docs/dev/table/streaming/time_attributes" >}})必须在一定的时间限制中（即时间窗口），那么就称之为时间区间 Join。
...
<span class="label label-danger">注意</span> 时态表有两种方式去定义，即 [时态表函数]({{< ref "docs/dev/table/streaming/versioned_tables" >}}#时态表函数) 和 [时态表 DDL]({{< ref "docs/dev/table/streaming/versioned_tables" >}}#时态表)，使用时态表函数的时态表 join 只支持在 Table API 中使用，使用时态表 DDL 的时态表 join 只支持在 SQL 中使用。
请参考[时态表]({{< ref "docs/dev/table/streaming/versioned_tables" >}})页面获取更多关于时态表和时态表函数的区别。
...
### 基于事件时间的时态 Join
基于事件时间的时态表 join 使用(左侧输入/探针侧) 的 事件时间 去关联(右侧输入/构建侧) [版本表]({{< ref "docs/dev/table/streaming/versioned_tables" >}}#声明版本表) 对应的版本。
...
查看[声明版本视图]({{< ref "docs/dev/table/streaming/versioned_tables" >}}#声明版本视图) 获取更多的信息关于如何声明一张来自 append-only 流的版本表。
...
例如，在探针侧表新插入一条事件时间时间为 `12:30:00` 的记录，它将和构建侧表时间点为 `12:30:00` 的版本根据[时态表的概念]({{< ref "docs/dev/table/streaming/versioned_tables" >}})进行 Join 运算。
...
基于处理时间的时态表 join 使用任意表 (左侧输入/探针侧) 的 处理时间 去关联 (右侧输入/构建侧) [普通表]({{< ref "docs/dev/table/streaming/versioned_tables" >}}#声明普通表)的最新版本.
...
时态表函数 Join 连接了一个递增表（左输入/探针侧）和一个时态表（右输入/构建侧），即一个随时间变化且不断追踪其改动的表。请参考[时态表]({{< ref "docs/dev/table/streaming/versioned_tables" >}})的相关章节查看更多细节。
...
如果没有[时态表]({{< ref "docs/dev/table/streaming/versioned_tables" >}})概念，则需要写一段这样的查询：
...
在[定义时态表函数]({{< ref "docs/dev/table/streaming/versioned_tables" >}}#defining-temporal-table-function)之后就可以使用了。时态表函数可以和普通表函数一样使用。
