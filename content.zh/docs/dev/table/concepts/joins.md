...
如果没有[时态表]({{< ref "docs/dev/table/concepts/versioned_tables" >}})概念，则需要写一段这样的查询：
...
在[定义时态表函数]({{< ref "docs/dev/table/concepts/versioned_tables" >}}#defining-temporal-table-function)之后就可以使用了。时态表函数可以和普通表函数一样使用。
