...
(参考 [Working with State]({{< ref "/dev/stream/state/state" >}})) 暴露给外部，从而用户可以在 Flink 外部查询作业 state。
...
客户端连接到一个代理，并发送请求获取特定 `k` 对应的 state。 如 [Working with State]({{< ref "/dev/stream/state/state.zh" >}}) 所述，keyed state 按照
...
从 [Flink distribution](https://flink.apache.org/downloads.html "Apache Flink: Downloads") 的 `opt/` 目录拷贝到 `lib/` 目录；
 2. 将参数 `queryable-state.enable` 设置为 `true`。详细信息以及其它配置可参考文档 [Configuration]({{< ref "/deployment/config.zh" >}}#queryable-state)。
...
(参考 [Using Managed Keyed State]({{< ref "/dev/stream/state/state.zh" >}}#using-managed-keyed-state))
...
关于依赖的更多信息, 可以参考如何 [配置 Flink 项目]({{< ref "/dev/project-configuration.zh" >}}).
