...
请注意，此页面的信息只与 Flink 自己生成的状态序列化器相关 [类型序列化框架]({{< ref "/dev/types_serialization" >}})。
...
这一过程是被 Flink 的类型序列化框架生成的序列化器透明处理的（[下面]({{< ref "/dev/stream/state/schema_evolution" >}}#数据结构升级支持的数据类型) 列出了当前的支持范围）。

如果你想要为你的状态类型实现自定义的 `TypeSerializer` 并且想要学习如何实现支持状态数据结构升级的序列化器，
可以参考 [自定义状态序列化器]({{< ref "/dev/stream/state/custom_serialization" >}})。
...
更多的迁移过程细节不在本文档谈论的范围；可以参考[文档]({{< ref "/dev/stream/state/custom_serialization" >}})。
...
Flink 基于下面的规则来支持 [POJO 类型]({{< ref "/dev/types_serialization" >}}#pojo-类型的规则)结构的升级:
