...
For fault-tolerant state, the `ProcessFunction` gives access to Flink's [keyed state]({{< ref "/dev/stream/state/state" >}}), accessible via the
`RuntimeContext`, similar to the way other stateful functions can access keyed state.

The timers allow applications to react to changes in processing time and in [event time]({{< ref "/dev/event_time" >}}).
