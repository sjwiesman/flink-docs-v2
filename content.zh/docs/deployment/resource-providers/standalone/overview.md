...
请参考 [配置参数页面]({{< ref "/deployment/config.zh" >}}) 获取更多细节以及额外的配置项。

特别地，

* 每个 JobManager 的可用内存值（`jobmanager.memory.process.size`），
* 每个 TaskManager 的可用内存值 （`taskmanager.memory.process.size`，并检查 [内存调优指南]({{< ref "/deployment/memory/mem_tuning.zh" >}}#configure-memory-for-standalone-deployment)），
...
In order to enable HA for a standalone cluster, you have to use the [ZooKeeper HA services]({{< ref "/deployment/ha/zookeeper_ha.zh" >}}).
