...
这里注意，Flink 的 [Table 和 SQL API]({{< ref "/dev/table/index" >}}) 完全可以满足很多 ETL 使用场景。但无论你最终是否直接使用 DataStream API，对这里介绍的基本知识有扎实的理解都是有价值的。
...
* **可查询性**: Flink 状态可以通过使用 [状态查询 API]({{< ref "/dev/stream/state/queryable_state" >}}) 从外部进行查询。
...
也可以选择使用 [状态的过期时间（TTL）]({{< ref "/dev/stream/state/state" >}}#state-time-to-live-ttl)，为状态描述符配置你想要旧状态自动被清除的时间。

### Non-keyed State

在没有键的上下文中我们也可以使用 Flink 管理的状态。这也被称作 [算子的状态]({{< ref "/dev/stream/state/state" >}}#operator-state)。它包含的接口是很不一样的，由于对用户定义的函数来说使用 non-keyed state 是不太常见的，所以这里就不多介绍了。这个特性最常用于 source 和 sink 的实现。
...
- [数据流转换]({{< ref "/dev/stream/operators/index" >}}#datastream-transformations)
- [有状态流的处理]({{< ref "/concepts/stateful-stream-processing" >}})
