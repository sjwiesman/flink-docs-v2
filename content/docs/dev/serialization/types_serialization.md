...
* **Registering custom serializers:** Flink falls back to [Kryo](https://github.com/EsotericSoftware/kryo) for the types that it does not handle transparently
  ...
  Additional Kryo serializers are available in many libraries. See [Custom Serializers]({{< ref "/dev/custom_serializers" >}}) for more details on working with custom serializers.
