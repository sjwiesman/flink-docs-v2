...
to the specified [state backend]({{< ref "/ops/state/state_backends" >}}) from where it will be retrieved when 
...
[Data Streaming Fault Tolerance]({{< ref "/learn-flink/fault_tolerance" >}}).
...
2. when resuming from a [savepoint]({{< ref "/ops/state/savepoints" >}}). 
