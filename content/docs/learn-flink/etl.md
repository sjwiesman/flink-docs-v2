...
Note that Flink's [Table and SQL APIs]({{< ref "/dev/table/index" >}})
...
* **queryable**: Flink state can be queried externally via the [Queryable State API]({{< ref "/dev/stream/state/queryable_state" >}}).
...
[operator state]({{< ref "/dev/stream/state/state" >}}#operator-state). The
...
- [DataStream Transformations]({{< ref "/dev/stream/operators/index" >}}#datastream-transformations)
