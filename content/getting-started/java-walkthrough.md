...
This function takes in a request and sends a response to an external system (or [egress](/io-module/#egress)).
...
Whenever data is consumed from an external system (or [ingress](/io-module/#ingress)), it is routed to a specific function based on a given function type and identifier.
...
[Persisted value](/sdk/#persistence) is a special data type that enables stateful functions to maintain fault-tolerant state scoped to their identifiers, so that each instance of a function can track state independently.
